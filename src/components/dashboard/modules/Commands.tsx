...
export default function Commands() {
    ...
    const { data, isError, isLoading, error } = useQuery({
        ...
        queryFn: () => betterFetch<CommandsFetch>(`${env.NEXT_PUBLIC_API_URL}/dash/commands/${guild.id}`)
    })
    ...
    const toggleCommandStatus = (category: string, commandName: string) => {
        ...
        const commandIndex = newCommands[category].findIndex((cmd: ECommand) => cmd.name === commandName);
        if (commandIndex !== -1) {
            newCommands[category][commandIndex].disabled = !newCommands[category][commandIndex].disabled;
    ...
    const saveChanges = () => {
        ...
        const data = sortCommandsByStatus(savedCommands)

        console.log(data)


        betterFetch<CommandsFetch>(`${env.NEXT_PUBLIC_API_URL}/dash/update-commands/${guild.id}`, {
            method: "POST",
            body: data,
            onSuccess: () => {
    ...
    useEffect(() => {
        const changesDetected = JSON.stringify(savedCommands) !== JSON.stringify(updatedCommands);
    ...
    if (isError) return <ErrorView error={error || new Error("Unable to find commands.")} />

    return (
        <div className="flex flex-row gap-3 flex-wrap mx-[2rem] my-[4.25rem] w-full">
            <Accordion className="w-full" type="multiple" defaultValue={Object.keys(data?.data?.data as Record<string, ECommand[]>)}>
                {Object.keys(data?.data?.data as Record<string, ECommand[]>).map((cat, i) => {
                    const cmds = (updatedCommands as unknown as Record<string, ECommand[]>)[cat as keyof typeof updatedCommands] as unknown as ECommand[]
                    return <AccordionItem className="w-full" value={cat} key={i}>
                        <AccordionTrigger className="text-sm font-bold">{cat.toUpperCase()}</AccordionTrigger>
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger>
                                            <Card className="w-[12rem] h-[3rem] flex flex-row justify-between items-center p-[1rem]">
                                                ...
                                                <div>
                                                    <Switch className="cursor-pointer" checked={!cmd.disabled} onClick={() => {
            ...
            <motion.div
                ...
                <Card className="flex flex-row justify-between p-[1rem] items-center">
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="opacity-70 text-sm">Make sure to save your changes before quitting.</p>
                    </div>
                    <Button variant={"red"} className="cursor-pointer" onClick={() => saveChanges()}>Save Changes</Button>
                </Card>
            </motion.div>
        </div>
    )
