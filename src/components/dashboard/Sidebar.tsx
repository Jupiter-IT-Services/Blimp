import { formatNameForAvatar, GuildDefault, sortGuildsByAvailable } from "@/lib/utils"
...
export default function Sidebar(props: SidebarProps) {
    ...
    return (
        <Card className="flex h-screen flex-col gap-2 py-[2rem] px-[0.75rem]">
            <a href="/dashboard">
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger className="mb-[1rem] border-b border-b-card-foreground/20 pb-[1rem]">
                            <Avatar className="w-[45px] h-[45px] flex items-center cursor-pointer justify-center bg-default-accent">
                                ...
                                />}
                                <AvatarFallback>{formatNameForAvatar(props.user.name)}</AvatarFallback>
                            </Avatar>
                        </TooltipTrigger>
            ...
            {newGuilds.map((g, i) => <a key={i} href={g.available
                ? `/dashboard/${g.id}`
                : `https://discord.com/oauth2/authorize?client_id=1372669471377850458&permissions=8&integration_type=0&scope=bot+applications.commands&guild_id=${g.id}&disable_guild_select=true`} className="cursor-pointer">
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger>
                            <Avatar className="w-[45px] h-[45px] flex items-center cursor-pointer justify-center bg-default-accent">
