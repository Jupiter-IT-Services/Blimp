...
export const dash = new Elysia({
  ...
  .get(`/guild/:id`, ({ params }) => {
    ...
    let guildConf = await db.select().from(guildConfig).where(eq(guildConfig.id, params.id))

    if (!guildConf) {
      await db.insert(guildConfig).values({ id: params.id }).execute().then((r) => info(`Created guild config on dashboard req.`)).catch(e => err(`Failed to create guild config on dashboard request.`))
    }
    guildConf = await db.select().from(guildConfig).where(eq(guildConfig.id, params.id))

    if (!guildConf[0]) return new Response(JSON.stringify({
      ok: false,
      data: null
    }), {
      status: 500
    ...
    if (!guild) return new Response(JSON.stringify({
      ok: false,
    ...
    const currentConfig = await db.select().from(guildConfig).where(eq(guildConfig.id, id));
    if (!currentConfig || !currentConfig[0]) return new Response(JSON.stringify({
      ok: false,
    ...
    try {
      ...
      const updated_commands = updateDisabledCommands(currentConfig[0].disabledCommands, data.enabled, data.disabled);
      return await db.update(guildConfig).set({
        disabledCommands: updated_commands
      }).where(eq(guildConfig.id, id)).then(() => {
